...
`本任务将跳转至外部实验页面，全程约五到七分钟，不需要专业知识。进入页面后，请完整粘贴回眸数据平台提供的用户编号（编号长度不固定），用于核对答卷和发放报酬。完成全部问题后，页面会显示一个六位数字完成码；请复制或截图保存，并返回回眸数据平台提交。请勿重复作答；若中途关闭，请在原来的手机或电脑上重新打开，以继续作答。`
